  ...
  exec('export LC_ALL=C; /sbin/sysctl -a ; unset LC_ALL',{maxBuffer: 1024 * 10000}, function(error, stdout) {
    ...
    if (!error) {
      ...
      for(var i = 0; i< lines.length;i++){
        ...
        obj = buildObj(obj,key.split('.'),0,val);
      }
      return callback(obj);
...
var buildObj = function(obj,ary,index,value){
  //console.log(ary+ ' -- '+index+ ' '+value+' --- obj ->'+JSON.stringify(obj, true, 4));
  if(ary.length == index){
    return value;
    //return obj;
  }else{
    if(obj[ary[index]]==undefined){
      obj[ary[index]]={};
    }
    obj[ary[index]] = buildObj(obj[ary[index]],ary,index+1,value);
  }
  return obj;
